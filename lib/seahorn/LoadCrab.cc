...
namespace crab_llvm {
  ...
  private:
	...
	return op::bv::is_bvnum(e) ||
	  bind::isConst<BVSORT>(e) ||
	  /* we should see only these operators */
	  isOpX<BADD>(e) || isOpX<BSUB>(e) || isOpX<BMUL>(e) ||
	  isOpX<BSDIV>(e) || isOpX<BUDIV>(e) ||
	  isOpX<BSREM>(e) || isOpX<BUREM>(e);
      }
	...
	if (exp->arity() == 2) {
	  ...
	  if (isBv(left) && isBv(right)) {
	    ...
	    if (left_width != right_width) {
	      errs() << "ERROR: inconsistent bitvector width in LinConsToExprSem\n";
	      assert(false);
	      return nullptr;
  ...
  public:
    ...
    LinConToExprSem(OpSem& sem, BvWidthMap& map)
	...
	return VisitAction::changeDoKidsRewrite(exp, r);
      ...
      /** Comparison Op */
      else if (isOpX<EQ>(exp) || isOpX<NEQ>(exp) ||
	       isOpX<LEQ>(exp)|| isOpX<GEQ>(exp) ||
	       isOpX<LT>(exp) || isOpX<GT>(exp)) {
	return VisitAction::changeDoKidsRewrite(exp, r);	  	  
      }
