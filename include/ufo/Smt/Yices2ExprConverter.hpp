...
namespace seahorn {
  namespace yices {
	...
	term_t res = NULL_TERM;  /* XXX: note we are returning type_t sometimes; so we will either need another routine; or else more clauses */

	if (bind::isBVar(e)) {
	  // XXX:  this is destined to be bound!
	  type_t vartype = marshal(bind::type(e), yices, cache);
	  unsigned index = bind::bvarId(e);
	  // XXX: a yices variable or an uninterpreted term?
	}
	...
	return M::marshal(e, yices, cache);
      ...
      /** Array Select */
        ...
        //XXX: this is wrong; yices wants an int32_t as the first argument.
        res = yices_select(t1, t2);
      /** Array Const */
